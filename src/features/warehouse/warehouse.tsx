...
export const WarehouseCard: React.FC<WarehouseCardProps> = ({ warehouse, onManage }) => {
  const statusColors = {
    ...
    [WarehouseStatus.SHIFT_VACANCY]: "bg-rose-500",
    [WarehouseStatus.DEFECTIVE_BATCH]: "bg-orange-500",
  };
  ...
  return (
    ...
    >
      ...
      <div className="p-8">
        <h3 className="text-2xl font-font-headline font-bold text-[#000000] mb-4">{warehouse.name}</h3>
...
export const InvitationCard: React.FC<InvitationCardProps> = ({ invitation, onAccept, onDecline }) => {
  return (
    <div className="bg-[#ffffff] rounded-xl p-6 flex flex-col md:flex-row items-start md:items-center gap-6 shadow-sm border border-[#e5e7eb]/15 group hover:shadow-md transition-all duration-300">
      <div className="w-20 h-20 rounded-lg overflow-hidden shrink-0 bg-[#f8f9fa]">
        <img 
      ...
      <div className="flex md:flex-col gap-2 w-full md:w-auto">
        <button 
          ...
          className="flex-1 md:w-32 py-2.5 rounded-lg bg-[#0040a1] text-white text-sm font-bold shadow-sm hover:bg-[#0056d2] transition-all font-headline"
        >
          Accept
        </button>
        <button 
          onClick={() => onDecline(invitation.id)}
          className="flex-1 md:w-32 py-2.5 rounded-lg bg-[#f8f9fa] text-[#666666] text-sm font-bold hover:bg-[#e5e7eb]/20 transition-all font-headline"
        >
          Decline
        </button>
      </div>
...
export const CreateWarehousePlaceholder: React.FC<{ onClick: () => void }> = ({ onClick }) => {
  return (
    ...
    >
      ...
      <h3 className="text-2xl font-headline font-bold text-[#000000] mb-4">Create New Warehouse</h3>
      <p className="text-[#666666] text-center max-w-60 leading-relaxed font-medium">
